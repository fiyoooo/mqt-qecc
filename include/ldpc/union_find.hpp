...
#include "osd.hpp"
#include "robin_map/robin_map.hpp"
#include "robin_map/robin_set.hpp"
...
struct Cluster {
    ...
    Cluster(ldpc::bp::BpSparse& parity_check_matrix, int syndrome_index, Cluster** ccm, Cluster** bcm,
            tsl::robin_set<int>& planar_code_boundary_bits = EMPTY_INT_ROBIN_SET) : pcm(parity_check_matrix), planar_code_boundary_bits(planar_code_boundary_bits) {
        this->active     = true;
        this->valid      = false;
...
public:
    ...
    std::vector<uint8_t>&
    peel_decode(const std::vector<uint8_t>& syndrome,
