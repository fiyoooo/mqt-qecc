...
Cluster::Cluster(ldpc::bp::BpSparse& parity_check_matrix, int syndrome_index, Cluster** ccm, Cluster** bcm,
...
void Cluster::get_candidate_bit_nodes() {
...
int Cluster::add_bit_node_to_cluster(int bit_index) {
    auto bit_membership = this->global_bit_membership[bit_index];
    if (bit_membership == this)
        return 0; // if the bit is already in the cluster terminate.
...
void Cluster::merge_with_cluster(Cluster* cl2) {
...
int Cluster::grow_cluster(const std::vector<double>& bit_weights, int bits_per_step) {
    if (!this->active) {
        return 0;
    }
...
int Cluster::find_spanning_tree_parent(const int check_index) {
...
void Cluster::find_spanning_tree() {
    ...
    while (!synds.empty()) {
        ...
        if (leaf_node_index == -1) {
            ...
            for (auto& e : this->pcm.iterate_row(leaf_node_index)) {
                ...
                if (this->spanning_tree_bits.contains(bit_index))
                    break;
            }
        }
        for (auto& e : this->pcm.iterate_column(bit_index)) {
...
ldpc::bp::BpSparse Cluster::convert_to_matrix(const std::vector<double>& bit_weights) {
...
std::vector<int> Cluster::invert_decode(const std::vector<uint8_t>& syndrome, const std::vector<double>& bit_weights) {
...
std::vector<int> Cluster::maxsat_decode(const std::vector<uint8_t>& syndrome, const std::vector<double>& bit_weights) {
    auto   temp = this->convert_to_matrix(bit_weights);
    // TODO use gaussian if cluster too small
    // if (temp.n < idk) { return invert_decode(syndrome, bit_weights); }
...
void Cluster::print() {
    ...
    for (auto i : this->spanning_tree_bits)
        std::cout << i << " ";
    std::cout << std::endl;
    std::cout << "Spanning tree leaf nodes: ";
    for (auto i : this->spanning_tree_leaf_nodes)
        std::cout << i << " ";
    std::cout << std::endl;
    std::cout << "Contains boundary bits: " << this->contains_boundary_bits << std::endl;
    std::cout << "Boundary bits: ";
    for (auto i : this->planar_code_boundary_bits)
        std::cout << i << " ";
    std::cout << std::endl;
    std::cout << "........." << std::endl;
}

// Implementation for UfDecoder constructor and functions.
UfDecoder::UfDecoder(ldpc::bp::BpSparse& parity_check_matrix) : pcm(parity_check_matrix) {
...
std::vector<uint8_t>& UfDecoder::peel_decode(const std::vector<uint8_t>& syndrome,
...
std::vector<uint8_t>& UfDecoder::matrix_decode(const std::vector<uint8_t>& syndrome,
                                               const std::vector<double>&  bit_weights,
                                               int                         bits_per_step) {
    fill(this->decoding.begin(), this->decoding.end(), 0);
...
std::vector<uint8_t>& UfDecoder::maxsat_decode(const std::vector<uint8_t>& syndrome,
                                               const std::vector<double>&  bit_weights,
                                               int                         bits_per_step) {
    fill(this->decoding.begin(), this->decoding.end(), 0);
